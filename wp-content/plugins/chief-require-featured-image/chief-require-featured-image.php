<?php
/*
Plugin Name: CHIEF Require Featured Image
Plugin URI: http://pressupinc.com/wordpress-plugins/require-featured-image/
Description: Fork of the Require Featured Image plugin. Require featured images for all published posts; can turn on or off for each post type.
Author: Press Up
Version: 1.1.3-1.0
...
function rfi_enqueue_edit_screen_js( $hook ) {
    ...
    if ( in_array( $post->post_type, rfi_return_post_types() ) ) {
        ...
        wp_localize_script(
            ...
            'objectL10n',
            array(
                'jsWarningHtml' => __( '<strong>This entry has no featured image.</strong> You need to set a featured image before publishing.', 'require-featured-image' ),
            )
        );
    }
}
